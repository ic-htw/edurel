...
class YamlTranslationBuilder(RelSchemaTranslationBuilder):
    ...
    def build(self) -> str:
        ...
        for table in self.tables:
            lines.append(f"- tablename: {self._yaml_scalar(table['tablename'])}")
            lines.append("  columns:")
            for column in table["columns"]:
                lines.append(f"  - columnname: {self._yaml_scalar(column['columnname'])}")
                lines.append(f"    type: {self._yaml_scalar(column['type'])}")
                if column.get("nullable"):
                    lines.append("    nullable: true")
            lines.append("  primary_key:")
            for primary_key in table["primary_key"]:
                lines.append(f"  - {self._yaml_scalar(primary_key)}")
            ...
            if foreign_keys:
                ...
                for foreign_key in foreign_keys:
                    ...
                    if foreign_key.get("fkname"):
                        lines.append(f"    fkname: {self._yaml_scalar(foreign_key['fkname'])}")
                    lines.append("    sourcecolumns:")
                    for source_column in foreign_key["sourcecolumns"]:
                        lines.append(f"    - {self._yaml_scalar(source_column)}")
                    lines.append(
                        f"    targettable: {self._yaml_scalar(foreign_key['targettable'])}"
                    )
                    lines.append("    targetcolumns:")
                    for target_column in foreign_key["targetcolumns"]:
                        lines.append(f"    - {self._yaml_scalar(target_column)}")
        if self.datalists:
            lines.append("datalists:")
            for datalist in self.datalists:
                lines.append(f"- tablename: {self._yaml_scalar(datalist['tablename'])}")
                lines.append("  values:")
...
class SqlTranslationBuilder(RelSchemaTranslationBuilder):
    ...
    @classmethod
    def _sql_datalist_insert_statements(cls, datalist: DataList) -> list[str]:
        statements: list[str] = []
        for index, value in enumerate(datalist.values, start=1):
            statements.append(
                f"INSERT INTO {datalist.tablename} (ID, Description, IsValid, SortOrder) "
                f"VALUES ({index}, {cls._sql_string_literal(value)}, 1, {index});"
            )
        return statements
    ...
    def add_datalist(self, datalist: DataList) -> None:
        self.insert_statements.extend(self._sql_datalist_insert_statements(datalist))
...
class SqlTranslationBuilderFkExternal(SqlTranslationBuilder):
    def __init__(self) -> None:
        super().__init__()
        self.alter_statements: list[str] = []

    def start_schema(self, rel_schema: RelSchema) -> None:
        super().start_schema(rel_schema)
        self.alter_statements = []

    def add_foreign_key(self, table: Table, foreign_key: ForeignKey) -> None:
        constraint_sql = (
            f"ALTER TABLE {table.tablename}\n"
            f"  ADD {self._sql_foreign_key_constraint(foreign_key)};"
        )
...
class SqlTranslationBuilderFkInternal(SqlTranslationBuilder):
    def __init__(self) -> None:
        super().__init__()
        self.current_table_comments: list[str] = []

    def start_table(self, table: Table) -> None:
        super().start_table(table)
        self.current_table_comments = []

    def add_foreign_key(self, table: Table, foreign_key: ForeignKey) -> None:
        constraint_sql = f"  {self._sql_foreign_key_constraint(foreign_key)}"
