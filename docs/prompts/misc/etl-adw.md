# Creation of an ETL process
you are an expert business intelligence spezialist.

# Source schema
the source is a classical OLTP database
```yaml
tables:
- tablename: Address
  columns:
...
# Target schema
The target is a classical star/snowflake data warehouse schema
```yaml
tables:
...
```
# Task
- create SQL statements that transform the source data such that it fits with the target schema 
- provide select-insert statements that fill the tables in the target schema
- put the complete sql code into file etl.sql in directory databases/adw-olap-small